...
**Stack:** Laravel 11 + Odoo 17 Community + OCA Payroll
**Working dir:** `/Users/haniyousif/dev/milejet-space/hr/hr-system`
...
- **Read path:** Laravel UI reads from local SQLite (fast).
...
attendances        — local cache of hr.attendance
...
| Employee login | The login flow validates against Odoo `res.users`. Of 34 imported employees, none have `res.users` accounts — they can't log in to either Odoo or this Laravel app. Three options documented below. |
...
1. **Employee login** — three options:
   - (A) Local Laravel auth + password setup link
   - (B) **OTP via email/SMS** (recommended for non-tech workforce)
   - (C) Create one `res.users` per employee in Odoo
