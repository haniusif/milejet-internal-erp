...
@section('content')
    <h1 class="text-2xl font-bold mb-4">{{ __('Edit: :name', ['name' => $employee->name]) }}</h1>
    <form method="POST" action="{{ route('employees.update', $employee->id) }}" class="bg-white rounded shadow p-6 space-y-4">
        ...
        <div class="flex gap-2 pt-4 border-t">
            <button class="bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-2 rounded">{{ __('Save changes') }}</button>
