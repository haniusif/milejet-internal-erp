...
<div class="flex flex-wrap items-end justify-between gap-3 mb-5">
    <div>
        <p class="text-xs uppercase tracking-wider text-slate-400">{{ __('Human Resources') }}</p>
...
<form method="GET" class="bg-white border border-slate-200 rounded-xl p-3 mb-5 flex flex-wrap items-center gap-2">
    <div class="relative flex-1 min-w-[180px]">
        ...
        <input type="text" name="q" value="{{ request('q') }}" placeholder="{{ __('Search by name, email, or job title...') }}"
    ...
    @if (request('q') || request('department_id'))
        <a href="{{ route('employees.index') }}" class="text-xs text-slate-500 hover:text-slate-700">{{ __('Reset') }}</a>
    @endif
</form>
                    ...
                    <td class="px-4 py-3 text-end whitespace-nowrap">
                        ...
                        @can('employees.write')
                            <a href="{{ route('employees.edit', $e->id) }}"
                               class="inline-flex items-center text-xs text-brand-600 hover:text-brand-700 hover:underline ms-2">{{ __('Edit') }}</a>
                        @endcan
